...
export function serializeExecution(execution: ExecutionWithDetails): Record<string, unknown> {
  ...
  return {
    ...
    triggered_by_user_id: execution.triggeredByUserId,
    status: execution.status,
    received_at: execution.receivedAt.toISOString(),
    started_at: execution.startedAt?.toISOString() ?? null,
    completed_at: execution.completedAt?.toISOString() ?? null,
    duration_ms: durationMs,
    error_message: execution.errorMessage,
    trigger_type: execution.triggerType ?? null,
    webhook_path: execution.webhookPath ?? null,
    webhook_method: execution.webhookMethod ?? null,
    log_entries: execution.logEntries?.map((log) => ({
      id: log.id,
      execution_id: log.executionHistoryId,
