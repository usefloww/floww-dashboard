...
async function fetchTypeDefinitions(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch type definitions for ${packageName}@${version}: ${response.statusText}`
    );
  }
...
${typeContent}
  ...
  console.log(`[Monaco] Added types for "${packageName}"`);
}
  ...
  await Promise.all(
    packagesToLoad.map(async (config) => {
      ...
      try {
        ...
        console.error(`Failed to prefetch types for ${name}@${version}:`, error);
      }
    })
  );
  ...
  await Promise.all(
    packagesToLoad.map(async (config) => {
      ...
      try {
        ...
        console.error(`Failed to load types for ${name}@${version}:`, error);
      }
    })
  );
}
